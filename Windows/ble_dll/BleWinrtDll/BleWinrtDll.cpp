...
fire_and_forget ReadDataAsync(BLEData data, condition_variable* signal, ReadDataResult* result) {
	...
	try {
		...
		if (characteristic != nullptr) {
			GattReadResult status = co_await characteristic.ReadValueAsync();
			
			if (status.Status() != GattCommunicationStatus::Success)
			{
				saveError(L"%s:%d Error reading", __WFILE__, __LINE__);
			}
			else
			{
				memcpy(result->buf, status.Value().data(), status.Value().Length());
				result->size = status.Value().Length();
				result->result = true;
			}
			
		} else {
			saveError(L"%s:%d characteristic nullptr", __WFILE__, __LINE__);
		}
	}
	catch (hresult_error& ex)
	{
...
void ReadData(BLEData* data, ReadDataResult* result) {
	mutex _mutex;
	unique_lock<mutex> lock(_mutex);
	condition_variable signal;
	ReadDataAsync(*data, &signal, result);
	signal.wait(lock);
}
